...
<!-- 상단 CSS, 부트스트랩, JS, jQuery, meta 등등 -->
...
<title>Truck Open Date List.jsp</title>
<style>
.btn-link{
...
<body class="bg-light-gray index">
	...
	<div class="container">
		<div class="row">
			<div class="col-lg-12 text-center">
				...
				<h3 class="section-subheading text-muted"
					style="margin-bottom: 20px">Food Truck By Open Date, 오늘은 어느
					트럭이 영업을 하고있을까요~?</h3>
			</div>
		</div>
		<!-- 테마별 버튼 -->
				...
				<h5 class="text-muted" style="font-style: italic">원하시는 테마로
					...
					<div style="display: inline-block">
						...
						<button type="button" class="btn btn-link"
							...
							style="font-size: 17px; background-color:#337ab7; color:white;">Open Date</button>
					</div>
		...
		<div class="row">
			<!-- Selct Date Choice -->
			<form action="truckOpenDateList.do" method="get">
				<input size="30" id="datepicker" type="text" name="openDate"
					class="btn btn-white" style="height: 40px;"
					placeholder="날짜를 기입해주세요" />
				<button type="submit" style="height: 40px; margin-bottom: 0px;"
		...
		<div class="row text-left" style="margin-bottom: 30px; margin-top:40px;">
			<h2 style="text-transform: none; font-family: arial;">Food Truck
				By Open Date</h2>
			<h3 class="text-muted" style="color: black;">
				오늘은 어느 트럭이 영업을 하고있을까요~? <br /> 위의 달력에서 날짜를 선택하시면 영업중인 푸드트럭 리스트가	출력됩니다.
			</h3>
		</div>
		<div class="row">
		<c:choose>
       		<c:when test="${empty openDateFoodList}">
				<!-- 가져온 위치 리스트가 없는 경우 -->
				<h3 style="text-align:center;">${errorMsg}</h3>
			</c:when>
			<c:otherwise>
	            <c:forEach items="${openDateFoodList }" var="row">
